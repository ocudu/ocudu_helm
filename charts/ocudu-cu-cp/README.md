...
```bash
helm install ocudu-cu-cp ./ -f values-o1.yaml
```
...
ManagedElement template with a `GNBCUCPFunction`): the adapter renders
`cu-cp-config.yml` from the NETCONF running config into a shared `emptyDir`, and
the entrypoint waits for that file (up to `CONFIG_CREATE_TIMEOUT`, 30s) before
starting `ocucp`. Liveness therefore tracks the adapter's `/config-healthy`
endpoint instead of `pgrep ocucp`.
