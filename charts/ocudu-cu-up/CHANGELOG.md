...
- `initCommand` configures the init process used as container PID 1 and defaults to `catatonit`, matching the gNB, CU and DU charts

## 1.5.0 (2026-08-06)

### Added

- `o1.o1Adapter.fileLog.{enabled,filename}`: optionally persist the o1 adapter container's stdout/stderr to a log file under `persistence.mountPath`. When enabled the container command is wrapped in a shell that tees the stream to the file (mounting the shared `ocudu-logs` volume, alongside the ocuup and netconf-server logs) while still forwarding it to pod stdout so `kubectl logs` keeps working. The adapter timestamps its own lines, so they are copied verbatim
...
- `topologySpreadConstraints` and `priorityClassName` values, for keeping the CU-UP schedulable across a node drain
- `startupProbe` (`enabled`, `periodSeconds`, `timeoutSeconds`, `failureThreshold`; `pgrep ocuup`, 180s budget by default) which gates the liveness and readiness probes in both plain and O1 mode
...
- `ocudu-cu-up.mainImage` template helper
...
- `image.tag: ""` (the shipped default) rendered the invalid reference `registry.gitlab.com/ocudu/ocudu/images/cu-up:` — the tag now falls back to the chart `appVersion`, as `values.yaml` already documented with `@default -- Chart appVersion`
