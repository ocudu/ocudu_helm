...
- `runtimeClassName`: run the DU pod under a specific RuntimeClass, for example the RuntimeClass generated by an OpenShift PerformanceProfile
- `EXPECTED_CPUS` is derived from `resources.requests.cpu` and passed to the entrypoint

### Fixed

- The entrypoint fails closed when it cannot determine an exclusive CPU set instead of falling back to every online CPU, and DPDK core mappings are rewritten onto the CPUs the pod actually owns. Previously an empty cgroup v2 `cpuset.cpus` could make the DU claim all online CPUs, including reserved ones
