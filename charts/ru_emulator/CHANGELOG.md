...
- `runtimeClassName`: run the RU emulator pod under a specific RuntimeClass, for example the RuntimeClass generated by an OpenShift PerformanceProfile
- `EXPECTED_CPUS` is derived from `resources.requests.cpu` and passed to the entrypoint

### Fixed

- `replicaCount` is honoured by the Deployment; it was previously ignored, so the replica count was always 1
- The entrypoint fails closed when it cannot determine an exclusive CPU set instead of falling back to every online CPU, and DPDK core mappings are rewritten onto the CPUs the pod actually owns
